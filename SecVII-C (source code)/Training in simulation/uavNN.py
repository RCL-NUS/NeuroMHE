...
class Net(nn.Module):
    def __init__(self, D_in, D_h, D_out):
        ...
        self.linear1 = nn.Linear(D_in, D_h) # The sim-2-real gap influence the decision to use the normalizaton for the first linear layer. 
        # 1. If we do not simulate the motor latency (which is the main factor for the sim-2-real gap) using the first-order LPF, we need to remove the normalization of the first layer;
        # 2. If we account for the latency in simulation, the presence or absence of normaliztion in the first layer has a negligible impact on the training result.
