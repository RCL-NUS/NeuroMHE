...
class Controller:
    ...
    def position_ctrl(self,x,Rb,ref_p,ref_v,ref_a,df_Imh):
    ...
    def L1_adaptive_law(self,z,Rb,z_hat):
    ...
    # Disturbance model 
    def dis(self,p,L_prev,v_lpf_prev,L0):
        K_stiff = 50 # stiffness of the elastic band, 1e3 for training
        ct      = 0.1 # damping coefficient
        p_offset = np.array([[0,0,0.175]]).T
        p_band   = p - p_offset
        dir_band = -p_band/LA.norm(p_band)
        length   = LA.norm(p_band)
        v_band   = (length-L_prev)/self.dt
        time_lpf = 0.05
        v_band_lpf = self.lowpass_filter(time_lpf,v_band,v_lpf_prev)
        if length > L0:
            norm_tension = K_stiff *( (length - L0) + ct * v_band_lpf)
        else:
            norm_tension = 0
...
class KF_gradient_solver:
    def __init__(self, xp, para):
        ...
        self.gt    = SX.sym('gt',6,1)
        self.xp     = xp
        self.x      = self.xp[0:6]
        self.traj_e = self.x - self.gt
        w_p, w_v  = 1, 0.5
        weight      = np.array([w_p, w_p, w_p, w_v, w_v, w_v])
    ...
    def loss_horizon(self, xmhe_traj, Gt, N, time):
        ...
        for k in range(horizon):
            x_mhe = xmhe_traj[k, :]
            dloss_track = Kloss * self.loss_tracking(x_mhe, Gt[len(Gt)-horizon+k])
    ...
    def ChainRule(self, Gt, xmhe_traj, X_opt):
        ...
        # Initialize the loss
        loss_track = 0
        Kloss = 1e2 #50 phase 1 1 phase 2
        ...
        for t in range(self.horizon):
            ...
            loss_track +=  dloss_track
            dlds =  Kloss * Ddlds_fn(xp0=x_mhe, gt0=Gt[len(Gt)-self.horizon+t])['dldsf'].full()
            dxdp = X_opt[t]
