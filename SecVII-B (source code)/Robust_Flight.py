...
class Controller:
    ...
    """
    def __init__(self, uav_para, ctrl_gain, x, dt_sample):
    ...
    def geometric_ctrl(self,x,x_prev,a_lpf_prev,j_lpf_prev,ref_p,ref_v,ref_a,ref_j,ref_s,b1_d,df_Imh,dtau_mh):
        ...
        b3c = Fd/LA.norm(Fd) # b3c = -A/norm(A), so A = -Fd. Time-derivative of df_Imh is omited.
        b2c = np.matmul(self.skew_sym(b3c), b1_d)/LA.norm(np.matmul(self.skew_sym(b3c), b1_d)) # b2c = -C/norm(C), so C = skew_sym(b1_d)@b3c, corresponding to Eq.(36) in [1].
    ...
    def geometric_ctrl_2(self,x,ref_p,ref_v,ref_a,ref_j,ref_s,b1_c,df_mh, dtau_mh):
        ...
        ev = v - ref_v
        # desired force in inertial frame
        Fd = -np.matmul(self.kp, ep) - np.matmul(self.kv, ev) + self.m*self.g*self.ez + self.m*ref_a - df_mh
        ...
        den = LA.norm(ref_a + self.g*self.ez)
    ...
    def L1_adaptive_law(self,x,z_hat):
        ...
        # The sign in front of 1/m*Rb*ez is positive here whereas it is negative in [3] due to the different definitions of the positive direction of inertial frame.
        ...
        B_R  = np.vstack((
            np.hstack((1/self.m*np.matmul(Rb,self.ez), np.zeros((3,3)))),
            np.hstack((np.zeros((3,1)), LA.inv(self.J)))
        ))
        B_Rp = np.vstack((
            np.hstack((1/self.m*np.matmul(Rb,self.ex), 1/self.m*np.matmul(Rb,self.ey))),
            np.hstack((np.zeros((3,1)), np.zeros((3,1))))
        ))
        B_bar = np.hstack((B_R, B_Rp))
        a_s = np.array([[-5,-5,-5,-10,-10,-10]])
        A_s = np.diag(a_s[0]) # diagonal Hurwitz matrix, same as that used in [3]
        PHI = np.matmul(LA.inv(A_s),(sLA.expm(self.dt*A_s)-np.identity(6)))
        mu  = np.matmul(sLA.expm(self.dt*A_s),(z_hat-z))
        sigma_hat  = -np.matmul(LA.inv(B_bar),np.matmul(LA.inv(PHI),mu))
        sig_hat_m  = np.reshape(sigma_hat[0:4,0],(4,1))
        sig_hat_um = np.reshape(sigma_hat[4:6,0],(2,1))
...
class MHE:
    ...
    def MHEsolver(self, Y, x_hat, xmhe_traj, ctrl, noise_traj, weight_para, time):
        assert hasattr(self, 'state'), "Define the state variable first!"
        assert hasattr(self, 'noise'), "Define the noise variable first!"
    ...
    def diffKKT_general(self):
        ...
        self.ddL0xp_fn    = Function('ddL0xp', [self.state, self.costate, self.ctrl, self.noise, self.measurement, self.weight_para, self.horizon1, self.index], [self.ddL0xp], ['x0', 'c0', 'u0', 'n0', 'm0', 'tp', 'h1', 'ind'], ['ddL0xpf'])
        self.ddLbar0xx    = jacobian(self.dLbar0x, self.state)
        self.ddLbar0xx_fn = Function('ddL0xx', [self.state, self.costate, self.ctrl, self.noise, self.measurement, self.weight_para, self.horizon1, self.index], [self.ddLbar0xx], ['x0', 'c0', 'u0', 'n0', 'm0', 'tp', 'h1', 'ind'], ['ddL0xxf'])
        self.ddLbar0xw    = jacobian(self.dLbar0x, self.noise)
        self.ddLbar0xw_fn = Function('ddL0xw', [self.state, self.costate, self.ctrl, self.noise, self.measurement, self.weight_para, self.horizon1, self.index], [self.ddLbar0xw], ['x0', 'c0', 'u0', 'n0', 'm0', 'tp', 'h1', 'ind'], ['ddL0xwf'])
        self.ddLbar0ww    = jacobian(self.dLbar0w, self.noise)
        self.ddLbar0ww_fn = Function('ddL0ww', [self.state, self.costate, self.ctrl, self.noise, self.measurement, self.weight_para, self.horizon1, self.index], [self.ddLbar0ww], ['x0', 'c0', 'u0', 'n0', 'm0', 'tp', 'h1', 'ind'], ['ddL0wwf'])
        self.ddLbar0wp    = jacobian(self.dLbar0w, self.weight_para)
        self.ddLbar0wp_fn = Function('ddL0wp', [self.state, self.costate, self.ctrl, self.noise, self.measurement, self.weight_para, self.horizon1, self.index], [self.ddLbar0wp], ['x0', 'c0', 'u0', 'n0', 'm0', 'tp', 'h1', 'ind'], ['ddL0wpf'])
        ...
        self.ddLbarxp_fn  = Function('ddLxp', [self.state, self.costate, self.ctrl, self.noise, self.measurement, self.weight_para, self.horizon1, self.index], [self.ddLbarxp], ['x0', 'c0', 'u0', 'n0', 'm0', 'tp', 'h1', 'ind'], ['ddLxpf'])
        ...
        self.ddLbarww_fn  = Function('ddLww', [self.state, self.costate, self.ctrl, self.noise, self.measurement, self.weight_para, self.horizon1, self.index], [self.ddLbarww], ['x0', 'c0', 'u0', 'n0', 'm0', 'tp', 'h1', 'ind'], ['ddLwwf'])
        self.ddLbarwp     = jacobian(self.dLbarw, self.weight_para) 
        self.ddLbarwp_fn  = Function('ddLwp', [self.state, self.costate, self.ctrl, self.noise, self.measurement, self.weight_para, self.horizon1, self.index], [self.ddLbarwp], ['x0', 'c0', 'u0', 'n0', 'm0', 'tp', 'h1', 'ind'], ['ddLwpf'])
    ...
    def GetAuxSys_general(self, state_traj_opt, costate_traj_opt, noise_traj_opt, weight_para, Y, ctrl):
        ...
        for k in range(horizon-1):
            ...
            if k == 0: # note that P is included only in the arrival cost
                matddLxx += [self.ddLbar0xx_fn(x0=curr_s, c0=curr_cs, u0=curr_c, n0=curr_n, m0=curr_m, tp=weight_para, h1=horizon-1, ind=k)['ddL0xxf'].full()]
                matddLxp += [self.ddL0xp_fn(x0=curr_s, c0=curr_cs, u0=curr_c, n0=curr_n, m0=curr_m, tp=weight_para, h1=horizon-1, ind=k)['ddL0xpf'].full()]
                matddLxw += [self.ddLbar0xw_fn(x0=curr_s, c0=curr_cs, u0=curr_c, n0=curr_n, m0=curr_m, tp=weight_para, h1=horizon-1, ind=k)['ddL0xwf'].full()]
                matddLww += [self.ddLbar0ww_fn(x0=curr_s, c0=curr_cs, u0=curr_c, n0=curr_n, m0=curr_m, tp=weight_para, h1=horizon-1, ind=k)['ddL0wwf'].full()]
                matddLwp += [self.ddLbar0wp_fn(x0=curr_s, c0=curr_cs, u0=curr_c, n0=curr_n, m0=curr_m, tp=weight_para, h1=horizon-1, ind=k)['ddL0wpf'].full()]
            else:
                ...
                matddLxp += [self.ddLbarxp_fn(x0=curr_s, c0=curr_cs, u0=curr_c, n0=curr_n, m0=curr_m, tp=weight_para, h1=horizon-1, ind=k)['ddLxpf'].full()]
                ...
                matddLww += [self.ddLbarww_fn(x0=curr_s, c0=curr_cs, u0=curr_c, n0=curr_n, m0=curr_m, tp=weight_para, h1=horizon-1, ind=k)['ddLwwf'].full()]
                matddLwp += [self.ddLbarwp_fn(x0=curr_s, c0=curr_cs, u0=curr_c, n0=curr_n, m0=curr_m, tp=weight_para, h1=horizon-1, ind=k)['ddLwpf'].full()]
...
class KF_gradient_solver:
    ...
    def loss_horizon(self, xmhe_traj, ref, N, time):
        loss_track = 0
        if time < N:
            horizon = time + 1
        else:
        ...
        for k in range(horizon):
            x_mhe = xmhe_traj[k, :]
            dloss_track = self.Kloss * self.loss_tracking(x_mhe, ref[len(ref)-horizon+k])
    ...
    def ChainRule(self, ref, xmhe_traj, X_opt):
        ...
        for t in range(self.horizon):
            ...
            dloss_track = self.Kloss * self.loss_tracking(x_mhe, ref[len(ref)-self.horizon+t])
            loss_track +=  dloss_track
            dlds =  self.Kloss * Ddlds_fn(xpa0=x_mhe, ref0=ref[len(ref)-self.horizon+t])['dldsf'].full()
            dxdp = X_opt[t]
