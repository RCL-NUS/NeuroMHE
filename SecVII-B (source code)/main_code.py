...
mode = 'e'  #input("enter 'train' or 'evaluate' without the quotation mark:")
print("=============================================")
...
def Train(epsilon0, gmin0, tunable_para0):
    ...
    while delta_loss >= eps:
        ...
        # record the tunable parameters in training
        tunable_para_training = np.zeros((N,D_out))
        for k in range(N):
            # Load the reference
            t_switch = 0
            ...
            sum_tp += weight_para
            tunable_para_training[k:k+1,:] = tunable_para
        ...
        Fullstate   += [State]
        K_iteration += [k_train]
        Tunable_para += [tunable_para_training]
        if estimator == 'a':
            PATH1 = "trained_data/trained_nn_model_reproduction.pt"
            torch.save(model_QR, PATH1)
        else:
            np.save('trained_data/tunable_para_trained_reproduction', tunable_para)
        if k_train == 0:
            eps = mean_loss/1e2
            if eps <=1:
                eps = 1
        ...
        if estimator=='a':
            np.save('trained_data/Loss_reproduction', Loss)
            np.save('trained_data/K_iteration_reproduction',K_iteration)
            np.save('trained_data/Position_train_reproduction',Position)
            np.save('trained_data/Dis_f_mh_train_reproduction',Dis_f_mh)
            np.save('trained_data/Dis_tau_mh_train_reproduction',Dis_tau_mh)
            np.save('trained_data/FULLSTATE_train_reproduction',FULLSTATE)
            np.save('trained_data/epsilon_trained_reproduction',epsilon)
            np.save('trained_data/gmin_trained_reproduction',gmin) 
            np.save('trained_data/Tunable_para_train_reproduction',Tunable_para)
        else:
            np.save('trained_data/Loss_dmhe_reproduction', Loss)
            np.save('trained_data/K_iteration_dmhe_reproduction',K_iteration)
            np.save('trained_data/Position_train_dmhe_reproduction',Position)
            np.save('trained_data/Dis_f_mh_train_dmhe_reproduction',Dis_f_mh)
            np.save('trained_data/Dis_tau_mh_train_dmhe_reproduction',Dis_tau_mh)
            np.save('trained_data/FULLSTATE_train_dmhe_reproduction',FULLSTATE)
            np.save('trained_data/epsilon_trained_dmhe_reproduction',epsilon)
            np.save('trained_data/gmin_trained_dmhe_reproduction',gmin)
            np.save('trained_data/Tunable_para_train_dmhe_reproduction',Tunable_para)
...
def Evaluate(key,key2):
    ...
    controller = key #input("enter 'a', or 'b',... without the quotation mark:")
    print("===============================================")
    print("Plot figures?")
    ...
    # Dis_f = np.zeros((3, N))
    # Dis_t = np.zeros((3, N))
    Dis_f = np.load('evaluation_data/Dis_f_l13.npy') # use the saved disturbance data generated offline on the unseen race-track trajectory.
    Dis_t = np.load('evaluation_data/Dis_t_l13.npy')
    # Dis_f = np.load('training_data/Dis_f.npy') 
    # Dis_t = np.load('training_data/Dis_t.npy')
    # record the disturbance estimates 
    df_MH = np.zeros((4, int(N/ratio)))
    dtau_MH = np.zeros((3, int(N/ratio)))
    # record the disturbance estimation errors 
    efxy_MH = np.zeros((1, int(N/ratio)))
    etauxy_MH = np.zeros((1, int(N/ratio)))
    ef_MH   = np.zeros((1, int(N/ratio)))
    etau_MH = np.zeros((1, int(N/ratio)))
    # record L1 estimation without filtering
    df_L1   = np.zeros((3, int(N/ratio)))
    dtau_L1 = np.zeros((3, int(N/ratio)))
    # record the inverse of standard deviation of the process noise 
    cov_f = np.zeros((3, N))
    cov_t = np.zeros((3, N))
    ...
    # record the tracking errors
    e_pxy = np.zeros((1, int(N/ratio)))
    e_p   = np.zeros((1, int(N/ratio)))
    # record the desired errors
    e_ref = np.zeros((1, int(N/ratio)))
    ...
    for k in range(N):
        ...
        if (k%ratio)==0: 
            ...
            efxy_MH[:,kmhe:kmhe+1] = LA.norm(df[0:2,0]-df_I_hat[0:2,0])
            ef_MH[:,kmhe:kmhe+1] = LA.norm(df-df_I_hat)
            dtau_MH[:,kmhe:kmhe+1] = dtau_hat 
            etauxy_MH[:,kmhe:kmhe+1] = LA.norm(dtau[0:2,0]-dtau_hat[0:2,0])
            etau_MH[:,kmhe:kmhe+1] = LA.norm(dtau-dtau_hat)
        ...
        # df, dtau = np.zeros((3,1)), np.zeros((3,1))
        euler_max = max(EULER[3,:])
        # if k == 400:
        #     df_z = df[2,0]
        #     if df_z > -10:
        #         flag = 1 
        if euler_max > 150 and k>400:
            flag2 = 1
    ...
    if controller == 'a':
        ...
        # np.save('evaluation_results/att_m',att_m)
        # np.save('evaluation_results/w_m',w_m)
        np.save('evaluation_data/Ref_p',Ref_p)
        np.save('evaluation_data/Dis_f'+str(key2),dis_f)
        np.save('evaluation_data/Dis_t'+str(key2),dis_t)
        np.save('evaluation_results/STATE'+str(key2),STATE)
        np.save('evaluation_results/df_MH'+str(key2),df_MH)
        np.save('evaluation_results/dtau_MH'+str(key2),dtau_MH)
        np.save('evaluation_results/Tunable_para_NeuroMHE',tp)
        np.save('evaluation_results/Q_k',Q_k)
        np.save('evaluation_results/p_gt_neuromhe'+str(key2),position)
        ...
        np.save('evaluation_results/STATE_dmhe'+str(key2),STATE)
        np.save('evaluation_data/Dis_f_dmhe'+str(key2),dis_f)
        np.save('evaluation_data/Dis_t_dmhe'+str(key2),dis_t)
        np.save('evaluation_results/df_MH_dmhe'+str(key2),df_MH)
        np.save('evaluation_results/dtau_MH_dmhe'+str(key2),dtau_MH)
        np.save('evaluation_results/Tunable_para_dmhe',tp)
        np.save('evaluation_results/Q_k_dmhe',Q_k)
        np.save('evaluation_results/p_gt_dmhe'+str(key2),position)
        ...
        np.save('evaluation_results/STATE_l1'+str(key2),STATE)
        np.save('evaluation_data/Dis_f_l1'+str(key2),dis_f)
        np.save('evaluation_data/Dis_t_l1'+str(key2),dis_t)
        np.save('evaluation_results/p_gt_l1'+str(key2),position)
        ...
        np.save('evaluation_results/df_MH_l1'+str(key2),df_MH)
        np.save('evaluation_results/dtau_MH_l1'+str(key2),dtau_MH)
    elif controller == 'd':
        np.save('evaluation_results/STATE_l1_active'+str(key2),STATE)
        np.save('evaluation_data/Dis_f_l1_active'+str(key2),dis_f)
        np.save('evaluation_data/Dis_t_l1_active'+str(key2),dis_t)
        np.save('evaluation_results/p_gt_l1_active'+str(key2),position)
        ...
        np.save('evaluation_results/df_MH_l1_active'+str(key2),df_MH)
        np.save('evaluation_results/dtau_MH_l1_active'+str(key2),dtau_MH)
    else:
    ...
    rmse_fx = format(mean_squared_error(df_MH[0,:], dis_f[0,:], squared=False),'.3f')
    rmse_fy = format(mean_squared_error(df_MH[1,:], dis_f[1,:], squared=False),'.3f')
    rmse_fz = format(mean_squared_error(df_MH[2,:], dis_f[2,:], squared=False),'.3f')
    rmse_fxy = format(mean_squared_error(efxy_MH[0,:], e_ref[0,:], squared=False),'.3f')
    rmse_f  = format(mean_squared_error(ef_MH[0,:], e_ref[0,:], squared=False),'.3f')
    rmse_tx = format(mean_squared_error(dtau_MH[0,:], dis_t[0,:], squared=False),'.3f')
    rmse_ty = format(mean_squared_error(dtau_MH[1,:], dis_t[1,:], squared=False),'.3f')
    rmse_tz = format(mean_squared_error(dtau_MH[2,:], dis_t[2,:], squared=False),'.3f')
    rmse_txy = format(mean_squared_error(etauxy_MH[0,:], e_ref[0,:], squared=False),'.3f')
    rmse_t  = format(mean_squared_error(etau_MH[0,:], e_ref[0,:], squared=False),'.3f')
    rmse_px = format(mean_squared_error(position[0,:], Ref_p[0,:], squared=False),'.3f')
    rmse_py = format(mean_squared_error(position[1,:], Ref_p[1,:], squared=False),'.3f')
    rmse_pz = format(mean_squared_error(position[2,:], Ref_p[2,:], squared=False),'.3f')
    rmse_pxy = format(mean_squared_error(e_pxy[0,:], e_ref[0,:], squared=False),'.3f')
    rmse_p = format(mean_squared_error(e_p[0,:], e_ref[0,:], squared=False),'.3f')
    rmse_mx = format(mean_squared_error(position[0,:], p_m[0,:], squared=False),'.3f')
    rmse_my = format(mean_squared_error(position[1,:], p_m[1,:], squared=False),'.3f')
    rmse_mz = format(mean_squared_error(position[2,:], p_m[2,:], squared=False),'.3f')
    rmse_mhx = format(mean_squared_error(position[0,:], p_MH[0,:], squared=False),'.3f')
    rmse_mhy = format(mean_squared_error(position[1,:], p_MH[1,:], squared=False),'.3f')
    rmse_mhz = format(mean_squared_error(position[2,:], p_MH[2,:], squared=False),'.3f')
    rmse_mvx = format(mean_squared_error(velocity[0,:], v_m[0,:], squared=False),'.3f')
    rmse_mvy = format(mean_squared_error(velocity[1,:], v_m[1,:], squared=False),'.3f')
    rmse_mvz = format(mean_squared_error(velocity[2,:], v_m[2,:], squared=False),'.3f')
    rmse_mhvx = format(mean_squared_error(velocity[0,:], v_MH[0,:], squared=False),'.3f')
    rmse_mhvy = format(mean_squared_error(velocity[1,:], v_MH[1,:], squared=False),'.3f')
    rmse_mhvz = format(mean_squared_error(velocity[2,:], v_MH[2,:], squared=False),'.3f')
    rmse_mroll= format(mean_squared_error(EULER[0,:], att_m[0,:], squared=False),'.3f')
    rmse_mpit = format(mean_squared_error(EULER[1,:], att_m[1,:], squared=False),'.3f')
    rmse_myaw = format(mean_squared_error(EULER[2,:], att_m[2,:], squared=False),'.3f')
    rmse_mhroll= format(mean_squared_error(EULER[0,:], att_MH[0,:], squared=False),'.3f')
    rmse_mhpit = format(mean_squared_error(EULER[1,:], att_MH[1,:], squared=False),'.3f')
    rmse_mhyaw = format(mean_squared_error(EULER[2,:], att_MH[2,:], squared=False),'.3f')
    rmse_mwx  = format(mean_squared_error(Omega[0,:], w_m[0,:], squared=False),'.3f')
    rmse_mwy  = format(mean_squared_error(Omega[1,:], w_m[1,:], squared=False),'.3f')
    rmse_mwz  = format(mean_squared_error(Omega[2,:], w_m[2,:], squared=False),'.3f')
    rmse_mhwx = format(mean_squared_error(Omega[0,:], w_MH[0,:], squared=False),'.3f')
    rmse_mhwy = format(mean_squared_error(Omega[1,:], w_MH[1,:], squared=False),'.3f')
    rmse_mhwz = format(mean_squared_error(Omega[2,:], w_MH[2,:], squared=False),'.3f')
    ...
    if plot == 'y':
        ...
        plt.figure(3)
        # plt.plot(Timemhe, dis_f[1,:], linewidth=1, linestyle='--')
        plt.plot(Timemhe, df_MH[3,:], linewidth=1)
        plt.xlabel('Time [s]')
        plt.ylabel('Disturbance force in xy axis')
        if controller == 'a':
            plt.legend(['Ground truth', 'NeuroMHE'])
            plt.grid()
            plt.savefig('evaluation_results/dfy_evaluation_NeuroMHE.png')
            ...
            plt.legend(['Ground truth', 'L1-AC'])
            plt.grid()
            plt.savefig('evaluation_results/dfxy_evaluation_L1AC.png')
        elif controller == 'd':
            plt.legend(['Ground truth', 'L1-AC'])
            plt.grid()
            plt.savefig('evaluation_results/dfy_evaluation_L1AC_active.png')
...
if mode =="train":
    ...
    # rmse_fxy = 1
    # rmse_pxy = 0.04
    # while rmse_fxy >=0.885 or rmse_pxy >=0.03:
    #     key = 'a'
    #     key2 = '3'
    #     rmse = Evaluate(key,key2)
    #     rmse_fxy = rmse[3,0].astype(np.float)
    #     rmse_pxy = rmse[13,0].astype(np.float)   
    # key = 'e'
    ...
    #     Rmse_pxy+= [rmse[13,0]]
    #     Rmse_p  += [rmse[14,0]]
    #     np.save('RMSE_boxplot_data/Rmse_fx_'+str(key),Rmse_fx)
    #     np.save('RMSE_boxplot_data/Rmse_fy_'+str(key),Rmse_fy)
    #     np.save('RMSE_boxplot_data/Rmse_fz_'+str(key),Rmse_fz)
    #     np.save('RMSE_boxplot_data/Rmse_fxy_'+str(key),Rmse_fxy)
    #     np.save('RMSE_boxplot_data/Rmse_f_'+str(key),Rmse_f)
    #     np.save('RMSE_boxplot_data/Rmse_tx_'+str(key),Rmse_tx)
    #     np.save('RMSE_boxplot_data/Rmse_ty_'+str(key),Rmse_ty)
    #     np.save('RMSE_boxplot_data/Rmse_tz_'+str(key),Rmse_tz)
    #     np.save('RMSE_boxplot_data/Rmse_txy_'+str(key),Rmse_txy)
    #     np.save('RMSE_boxplot_data/Rmse_t_'+str(key),Rmse_t)
    #     np.save('RMSE_boxplot_data/Rmse_px_'+str(key),Rmse_px)
    #     np.save('RMSE_boxplot_data/Rmse_py_'+str(key),Rmse_py)
    #     np.save('RMSE_boxplot_data/Rmse_pz_'+str(key),Rmse_pz)
    #     np.save('RMSE_boxplot_data/Rmse_pxy_'+str(key),Rmse_pxy)
    #     np.save('RMSE_boxplot_data/Rmse_p_'+str(key),Rmse_p)
