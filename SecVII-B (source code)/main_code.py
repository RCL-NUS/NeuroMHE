...
def Evaluate():
    ...
    if controller == 'a':
        np.save('trained_data/p_MH',p_MH)
        np.save('trained_data/v_MH',v_MH)
        np.save('trained_data/att_MH',att_MH)
        np.save('trained_data/w_MH',w_MH)
        np.save('trained_data/p_m',p_m)
        np.save('trained_data/v_m',v_m)
        np.save('trained_data/att_m',att_m)
        np.save('trained_data/w_m',w_m)
        np.save('trained_data/df_MH',df_MH)
        np.save('trained_data/dtau_MH',dtau_MH)
        np.save('trained_data/Tunable_para_NeuroMHE',tp)
        np.save('trained_data/Q_k',Q_k)
        np.save('trained_data/p_gt_neuromhe',position)
        np.save('trained_data/v_gt_neuromhe',velocity)
        np.save('trained_data/att_gt_neuromhe',EULER)
        np.save('trained_data/w_gt_neuromhe',Omega)
    elif controller == 'b':
