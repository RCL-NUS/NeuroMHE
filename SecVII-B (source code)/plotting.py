...
or relatively large (due to the nearly unstable flight).
Please note that the removal of the outliers from the figure in our paper does not affect the results and conclusion of SecVII-B-2).
...
for i, data in enumerate(datasets_f):
    ...
    k = i % len(colours)
    flierprops = dict(marker='o', markersize=0.2, markerfacecolor=colours[k], markeredgecolor=colours[k],
                  linestyle='none')
    bp = plt.boxplot(
        np.array(data), whis=[5, 95], widths=0.6 / len(datasets_f),
        labels=list(datasets_f[0]), patch_artist=True,
        positions=[x_pos[i] + j * 1 for j in range(len(data.T))],
        showfliers=False, flierprops=flierprops,notch =False,whiskerprops=dict(linestyle='-',linewidth=0.5),boxprops=dict(linestyle='--', linewidth=0.5,color=colours[k],facecolor=colours[k],alpha=0.7)
       
    ) # whis=[5, 95]
    for cap in bp['caps']: 
        cap.set(linewidth = 0.5)
    for median in bp['medians']: 
        median.set(linewidth = 0.5) 
    k = i % len(colours)
...
for i, data in enumerate(datasets_t):
    ...
    flierprops = dict(marker='o', markersize=0.2, markerfacecolor=colours[k], markeredgecolor=colours[k],
                  linestyle='none')
    bp = plt.boxplot(
        np.array(data), whis=[5, 95],widths=0.6 / len(datasets_t),
        labels=list(datasets_t[0]), patch_artist=True,
        positions=[x_pos[i] + j * 1 for j in range(len(data.T))],
        showfliers=False, flierprops=flierprops,notch =False,whiskerprops=dict(linestyle='-',linewidth=0.5),boxprops=dict(linestyle='--', linewidth=0.25,color=colours[k],facecolor=colours[k],alpha=0.7)
    )
    for cap in bp['caps']: 
        cap.set(linewidth = 0.5)
    for median in bp['medians']: 
    ...
    medians = [str(round(s, 3)) for s in medians]
    # Increase the height of the plot by 5% to fit the labels
    plt.ylim([y_min - 0.17 * y_range, y_max + 0 * y_range])
    # Set the y-positions for the labels
    y_pos = y_min - 0.145 * y_range
    for tick, label in zip(range(len(data.T)), plt.xticks()):
        # k = tick % 2
        plt.text(
            positions[tick], y_pos, fr'{medians[tick]}',
...
for i, data in enumerate(datasets_p):
    ...
    medians = [str(round(s, 3)) for s in medians]
    # Increase the height of the plot by 5% to fit the labels
    plt.ylim([y_min - 0.14 * y_range, y_max - 0.05* y_range])
    # Set the y-positions for the labels
    y_pos = y_min - 0.115 * y_range
    for tick, label in zip(range(len(data.T)), plt.xticks()):
        # k = tick % 2
        plt.text(
            positions[tick], y_pos, fr'{medians[tick]}',
