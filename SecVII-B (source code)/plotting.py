...
for i, data in enumerate(datasets_f):
    ...
    bp = plt.boxplot(
        ...
        positions=[x_pos[i] + j * 1 for j in range(len(data.T))],
        showfliers=True, flierprops=flierprops,notch =False,whiskerprops=dict(linestyle='-',linewidth=0.5),boxprops=dict(linestyle='--', linewidth=0.5,color=colours[k],facecolor=colours[k],alpha=0.5)
    ...
    k = i % len(colours)
    # for box in bp['boxes']:
    #     box.set(facecolor=colours[k], edgecolor='black',linewidth = 0.5)
        # box.set_edgecolor('black')
        # box.set_linewidth(0.5)
    # for element in ['boxes']:
    #     plt.setp(bp[element], color=colours[k],linewidth=0.5)
...
for i, data in enumerate(datasets_t):
    ...
    bp = plt.boxplot(
        ...
        positions=[x_pos[i] + j * 1 for j in range(len(data.T))],
        showfliers=True, flierprops=flierprops,notch =False,whiskerprops=dict(linestyle='-',linewidth=0.5),boxprops=dict(linestyle='--', linewidth=0.25,color=colours[k],facecolor=colours[k],alpha=0.5)
    )
    for cap in bp['caps']: 
        cap.set(linewidth = 0.5)
    for median in bp['medians']: 
