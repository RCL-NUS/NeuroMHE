...
for i, data in enumerate(datasets_f):
    ...
    bp = plt.boxplot(
        ...
        positions=[x_pos[i] + j * 1 for j in range(len(data.T))],
        showfliers=True, flierprops=flierprops,notch ='True',whiskerprops=dict(linestyle='-',linewidth=0.5),boxprops=dict(linestyle='--', linewidth=0.25,color='black',facecolor=colours[k])
...
for i, data in enumerate(datasets_p):
    ...
    flierprops = dict(marker='o', markersize=0.2, markerfacecolor=colours[k], markeredgecolor=colours[k],
                  linestyle='none')
    bp = plt.boxplot(
        np.array(data), widths=0.6 / len(datasets_p),
...
plt.yticks(np.arange(0,0.3,0.095),**font1)
ax.xaxis.set_minor_locator(matplotlib.ticker.FixedLocator(
