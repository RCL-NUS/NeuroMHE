...
def Train(epsilon0, gmin0):
    ...
    while delta_cost >= eps: #flag ==0
        ...
        for key in train_set:
            ...
            for j in range(n_start,n_start+4000): # these data points covers a wide range of velocity from 0.05 to 16.38 m/s 
    ...
    plt.grid()
    plt.savefig('plots_testset/mean_loss_train.png',dpi=600)
    # plt.show()
...
def Evaluate():
    ...
    for j in range(n_start,N_ev):
        ...
        dt_Bmh   = np.transpose(xmhe_traj[-1, 9:12])
        dt_Bmh   = np.reshape(dt_Bmh, (3, 1))
        wrench_mhe = np.vstack((df_Imh, dt_Bmh))
        ...
        acc = np.array([[accx_seq[n*j], accy_seq[n*j], accz_seq[n*j], angaccx_seq[n*j], angaccy_seq[n*j], angaccz_seq[n*j]]]).T
        df_t, dt_t = GroundTruth(w_B, acc, uav.m, uav.J)
        df_t      = np.reshape(np.matmul(R_B,df_t),(3,1)) # transformed to the world frame
        state_t       = np.vstack((df_t, dt_t))
        ground_truth += [state_t]
        ...
        print('sample=', j, 'Dis_x=', df_t[0, 0], 'df_Imh_x=', df_Imh[0, 0], 'Dis_y=', df_t[1, 0], 'df_Imh_y=', df_Imh[1, 0], 'Dis_z=', df_t[2, 0], 'df_Imh_z=', df_Imh[2, 0])
