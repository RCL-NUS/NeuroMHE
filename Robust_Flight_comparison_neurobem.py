...
class MHE:
    ...
    def MHEsolver(self, Y, m, xmhe_traj, weight_para, time, print_level=0):
        assert hasattr(self, 'state'), "Define the state variable first!"
        assert hasattr(self, 'noise'), "Define the noise variable first!"
    ...
    def diffKKT_general(self):
        ...
        self.ddL0xw_fn    = Function('ddL0xw', [self.state, self.costate, self.noise, self.measurement, self.weight_para, self.horizon1, self.index], [self.ddL0xw], ['x0', 'c0', 'n0', 'm0', 'tp', 'h1', 'ind'], ['ddL0xwf'])
        ...
        self.ddLbarxe_fn  = Function('ddLxe', [self.state, self.costate, self.noise, self.weight_para, self.horizon1, self.index], [self.ddLbarxe], ['x0', 'c0', 'n0', 'tp', 'h1', 'ind'], ['ddLxef'])
        self.ddLbaree     = jacobian(self.dLbare, self.noise) # k=t-N,...,t-1
        self.ddLbaree_fn  = Function('ddLee', [self.state, self.costate, self.noise, self.weight_para, self.horizon1, self.index], [self.ddLbaree], ['x0', 'c0', 'n0', 'tp', 'h1', 'ind'], ['ddLeef'])
        self.ddLbarew     = jacobian(self.dLbare, self.weight_para) # k=t-N,...,t-1
        self.ddLbarew_fn  = Function('ddLew', [self.state, self.costate, self.noise, self.weight_para, self.horizon1, self.index], [self.ddLbarew], ['x0', 'c0', 'n0', 'tp', 'h1', 'ind'], ['ddLewf'])
        
        # Second-order derivative of final Lagrangian, k=t
        self.ddLbarTxw    = jacobian(self.dLbarTx, self.weight_para)
        self.ddLbarTxw_fn = Function('ddLTxw', [self.state, self.costate, self.noise, self.weight_para, self.horizon1, self.index], [self.ddL0xw], ['x0', 'c0', 'n0', 'tp', 'h1', 'ind'], ['ddLTxwf'])
    ...
    def GetAuxSys_general(self, state_traj_opt, costate_traj_opt, noise_traj_opt, weight_para, Y):
        ...
        for k in range(horizon-1):
            ...
            if k == 0:
                matddLxw += [self.ddL0xw_fn(x0=curr_s, c0=curr_cs, n0=curr_n, m0=curr_m, tp=weight_para, h1=horizon-1, ind=k)['ddL0xwf'].full()]
            else:
                matddLxw += [self.ddLbarxw_fn(x0=curr_s, c0=curr_cs, n0=curr_n, m0=curr_m, tp=weight_para, h1=horizon-1, ind=k)['ddLxwf'].full()]
            matddLxe += [self.ddLbarxe_fn(x0=curr_s, c0=curr_cs, n0=curr_n, tp=weight_para, h1=horizon-1, ind=k)['ddLxef'].full()]
            matddLee += [self.ddLbaree_fn(x0=curr_s, c0=curr_cs, n0=curr_n, tp=weight_para, h1=horizon-1, ind=k)['ddLeef'].full()]
            matddLew += [self.ddLbarew_fn(x0=curr_s, c0=curr_cs, n0=curr_n, tp=weight_para, h1=horizon-1, ind=k)['ddLewf'].full()]
        ...
        curr_m    = Y[-1]
        matddLxx += [self.ddLbarxx_fn(x0=curr_s, c0=curr_cs, n0=curr_n, m0=curr_m, tp=weight_para, h1=horizon-1, ind=horizon-1)['ddLxxf'].full()]
        matddLxw += [self.ddLbarxw_fn(x0=curr_s, c0=curr_cs, n0=curr_n, m0=curr_m, tp=weight_para, h1=horizon-1, ind=horizon-1)['ddLxwf'].full()]
    ...
    # the following function computes the coefficient matrices, 
    # which are used to solve for the gradient trajectory by directly computing the inverse of the large KKT matrix [3]
    """
    [3] Esfahani, H.N., Kordabad, A.B. and Gros, S., 2021, May. 
        Reinforcement learning based on MPC/MHE for unmodeled and partially observable dynamics. 
        In 2021 American Control Conference (ACC) (pp. 2121-2126). IEEE.
    """
    def diffKKT_direct(self):
        X = SX.sym('X', self.horizon*self.n_state, 1)
        W = SX.sym('W', (self.horizon-1)*self.n_noise, 1)
        G = SX.sym('G', (self.horizon-1)*self.n_state, 1)
        for k in range(self.horizon-1):
            xk = SX.sym('X_' + str(k), self.n_state, 1)
            X[k*self.n_state:(k+1)*self.n_state, :] = xk
            wk = SX.sym('X_' + str(k), self.n_noise, 1)
            W[k*self.n_noise:(k+1)*self.n_noise, :] = wk
            gk = SX.sym('G_' + str(k), self.n_state, 1)
            G[k*self.n_state:(k+1)*self.n_state, :] = gk
...
class KF_gradient_solver:
    def __init__(self, xa, r11):
        self.n_xmhe = xa.numel()
        self.rN11   = r11
        self.x_t    = SX.sym('x_t',6,1)
        self.xa     = xa
        self.dismhe = vertcat(self.xa[6:9,0], self.xa[21:24,0]) 
    ...
    def loss_tracking(self, xpa, x_t):
        loss_fn = Function('loss', [self.xa, self.x_t], [self.loss], ['xpa0', 'gt0'], ['lossf'])
        loss_track = loss_fn(xpa0=xpa, gt0=x_t)['lossf'].full()
        return loss_track
    ...
    def ChainRule(self, gt, xmhe_traj, X_opt):
        ...
        for t in range(self.horizon):
            ...
            dloss_track = Kloss * self.loss_tracking(x_mhe, gt[len(gt)-self.horizon+t])
            loss_track +=  dloss_track
            dlds =  Kloss * Ddlds_fn(xpa0=x_mhe, gt0=gt[len(gt)-self.horizon+t])['dldsf'].full()
            dxdp = X_opt[t]
