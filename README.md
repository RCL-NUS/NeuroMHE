...
5. Check the RMSE results. The RMSEs can be computed under two conditions: A and B.
   ...
   * Under condition B, the force is expressed in the body frame (cond.B1), and the RMSEs of the planar and the overall disturbances are computed using the vector error (cond.B2), e.g., $\Delta_{f}=\sqrt{(d_{f_x}-\hat d_{f_x})^2 + (d_{f_y}-\hat d_{f_y})^2 + (d_{f_z}-\hat d_{f_z})^2}$. Furthermore, a mass of 0.752 kg is used for obtaining the ground truth force $\mathbf d_{f}$ (cond.B3).
   ...
   In the folder '**Check_RMSE**', run the Python file '**RMSE_Computation_Slow_Better_Cond_B.py**' to replicate the RMSE results presented in the above table and our paper. For completeness, we also provide the RMSE results of NeuroMHE trained on the fast training set under both conditions. To access these results, find the Python files '**RMSE_Computation_Cond_A.py**' and '**RMSE_Computation_Cond_B.py**' in the same '**Check_RMSE**' folder and run them respectively. Compared with the RMSE results from '**RMSE_Computation_Cond_B.py**', the estimation performance of NeuroMHE trained on the slow training set is better as we allow for more trainning episodes and slight adjust the weightings in the loss $L\left ( \mathbf{\hat{x}} \right )$ that evaluates the quality of the MHE solution trajectory.
