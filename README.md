...
Another tricky point worth noting is that the Python version can affect training stability. We find that using Python 3.10 with the solver parameters from the left column of the above table can lead to instability (workable IPOPT parameters for Python3.10: opts['ipopt.tol']=1e-14, opts['ipopt.max_iter']=3e3, and opts['ipopt.acceptable_tol']=1e-14). A good practice is to keep the version aligned with the one listed above (version 3.9) and fine-tune the IPOPT solver parameters.
