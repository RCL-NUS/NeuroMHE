...
5. Check the RMSE results. In the folder '**Check_RMSE**', run the Python file '**RMSE_vector_error_slowset.py**' to replicate the RMSE results presented in Table V of our paper. Additionally, we provide the RMSE results of NeuroMHE trained on the fast training set. To access these results, locate the Python files '**RMSE_scalar_error_v1.py**' and '**RMSE_vector_error.py**' in the same folder and run them respectively. The file names themselves imply the distinction between these two datasets: the former is derived using scalar error (i.e., $e_{f}=\sqrt{d_{f_x}^2+d_{f_y}^2+d_{f_z}^2} - \sqrt{ \hat d_{f_x}^2 + \hat d_{f_y}^2 + \hat d_{f_z}^2}$) and expresses the force in the world frame, while the latter is attained through vector error (i.e., $e_{f}=\sqrt{(d_{f_x}-\hat d_{f_x})^2 + (d_{f_y}-\hat d_{f_y})^2 + (d_{f_z}-\hat d_{f_z})^2}$) and presents the force in the body frame.
   
   Note that we refrain from utilizing the residual force data available in the NeuroBEM dataset to compute NeuroBEM's RMSE (See columns 36-38 in the file 'predictions.tar.xz', accessible at https://download.ifi.uzh.ch/rpg/NeuroBEM/). This decision is based on our confirmation that the provided residual force data was computed using the mass of 0.752 kg instead of the revised value of 0.772 kg. It is important to emphasize that the omission of this residual force data does not compromise the value of the remaining dataset; it serves purely for user convenience. Users can effortlessly calculate this data using the NeuroBEM's predicted forces and the measured acceleration, as explained on https://rpg.ifi.uzh.ch/neuro_bem/Readme.html. To verify the mass modification for each trajectory in the test dataset, run the MATLAB file '**residual_force_trajectory name.m**' located in the folder '**MATLAB_code_for_mass_verification**'. For example, running the MATLAB file '**residual_force_3D_Circle_2.m**' demenstrates that the NeuroBEM's RMSE for the overall force in the 3D Circle_2 trajectory, with a mass of 0.752 kg, is 0.28 (as indicated by the value of 'rmse_bemnn' in Workspace). In contrast, the RMSE value changes to 0.499 with a mass of 0.772 kg (as indicated by the 'rmse_bemnn_updated' value in Workspace).

   By comparing the RMSE results obtained from '**RMSE_vector_error.py**', we observe that NeuroMHE, trained on the slow training set, exhibits a better estimation performance. This improvement can be attributed to an increased number of training episodes and slight adjustments in the weightings of the loss function $L\left ( \mathbf{\hat{x}} \right )$ that evaluates the quality of the MHE solution trajectory by comparing with the ground truth. These findings highlight the exceptional robustness of our approach concerning variations in the training dataset.
