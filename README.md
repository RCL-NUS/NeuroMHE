...
![neuromhe_structure](https://github.com/RCL-NUS/NeuroMHE/assets/70559054/93479d8b-d9e5-48f0-aed3-dbad0dc00d21width=50%height=50%)
...
5. Check the RMSE results. In the folder '**Check_RMSE**', run the Python file '**RMSE_vector_error_slowtrainingset.py**' to replicate the RMSE results presented in Table V of our paper. Additionally, we provide the RMSE results of NeuroMHE trained on the fast training set. To access these results, locate the Python files '**RMSE_scalar_error_v1.py**' and '**RMSE_vector_error.py**' in the same folder and run them respectively. The file names themselves imply the distinction between these two datasets: the former is derived using scalar error (i.e., $e_{f}=\sqrt{d_{f_x}^2+d_{f_y}^2+d_{f_z}^2} - \sqrt{ \hat d_{f_x}^2 + \hat d_{f_y}^2 + \hat d_{f_z}^2}$) and expresses the force in the world frame, while the latter is attained through vector error (i.e., $e_{f}=\sqrt{(d_{f_x}-\hat d_{f_x})^2 + (d_{f_y}-\hat d_{f_y})^2 + (d_{f_z}-\hat d_{f_z})^2}$) and presents the force in the body frame.
   
   Note that we refrain from utilizing the residual force data available in the NeuroBEM dataset to compute NeuroBEM's RMSE (See columns 36-38 in the file 'predictions.tar.xz', accessible at https://download.ifi.uzh.ch/rpg/NeuroBEM/). This decision is based on our confirmation that the provided residual force data was computed using the mass of 0.752 kg instead of the revised value of 0.772 kg. *Nevertheless, it is important to emphasize that the omission of this residual force data does not compromise the value of the remaining dataset, as it serves purely for user convenience*. Users can effortlessly calculate this data using the NeuroBEM's predicted forces and the measured acceleration, as explained on https://rpg.ifi.uzh.ch/neuro_bem/Readme.html.

   To demonstrate the mass verification, run the MATLAB file '**residual_force_trajectory name.m**' located in the folder '**MATLAB_code_for_mass_verification**'. For example, running the MATLAB file '**residual_force_3D_Circle_2.m**' demenstrates that the NeuroBEM's RMSE for the overall force in the 3D Circle_2 trajectory, with the provided residual force data (calculated with the intially reported mass of 0.752 kg), is 0.28 (as indicated by the value of 'rmse_bemnn' in Workspace). In contrast, the RMSE value changes to 0.499 with the revised mass of 0.772 kg (as indicated by the 'rmse_bemnn_updated' value in Workspace). Additionally, we provide the RMSE values calculated with the initially reported mass solely for reference. To replicate these results, run the Python file '**RMSE_vector_error_slowtrainingset_m=0752.py**'.
