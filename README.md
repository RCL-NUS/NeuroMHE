...
5. Check the RMSE results (presented in the above table and our paper). The RMSEs can be computed under two conditions: condition A and condition B. That is, under condition A, the force is expressed in the world frame (cond.A1), and the RMSEs of the planar and the overall disturbances are computed using the scalar error (cond.A2), for example, $e_{f_{xy}}=\sqrt{d_{f_x}^2+d_{f_y}^2} - \sqrt{ \hat d_{f_x}^2+ \hat d_{f_y}^2}$. Additionally, a mass of 0.772 kg is used for obtaining the ground truth force (cond.A3). On the other hand, under condition B, the force is expressed in the body frame (cond.B1), and the RMSEs of the planar and the overall disturbances are computed using the vector error (cond.B2), for example, $e_{f_{xy}}=\sqrt{(d_{f_x}-\hat d_{f_x})^2 + (d_{f_y}-\hat d_{f_y})^2}$. Furthermore, a mass of 0.752 kg is used for obtaining the ground truth force (cond.B3).
