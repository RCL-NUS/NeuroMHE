...
5. Check the RMSE results (presented in the above table and our paper). The RMSEs can be computed under either condition A or condition B. That is, cond.A1: the force is expressed in the world frame, cond.A2: the RMSEs of the planar and the overall disturbances are computed using the scalar error (e.g., $e_{f_{xy}}=\sqrt{d_{f_x}^2+d_{f_y}^2} - \sqrt{ \hat d_{f_x}^2+ \hat d_{f_y}^2}$), cond.A3: the mass of 0.772kg is used for obtaining the ground truth force; cond.B1: the force is expressed in the body frame, cond.B2: the RMSEs of the planar and the overall disturbances are computed using the vector error (e.g., $e_{f_{xy}}=\sqrt{(d_{f_x}-\hat d_{f_x})^2 + (d_{f_y}-\hat d_{f_y})^2}$), cond.B3: the mass of 0.752kg is used for obtaining the ground truth force.
